var img, canvas, ctx, gridDrawn, maxi, maxj, x, y, grida, offx, offy, penx,
peny, peni, penj, coords, canvasOffsetX, canvasOffsetY, penMode,
modeColours = new Array(new Array(0, 0, 255, 0.3)),
modeNames = new Array('Normal'),
modeDisplay = new Array(true),
modeDefValue = new Array(true),
modeMasks = new Array(1),
canvasBorder = 2,
canvasFont = '11px sans-serif',
canvasFontSize = 11,
numModes = 1,
curMode = 0,
pen = false;
...
function autoDetect(x0, y0, x1, y1, amin, amax) {
	...
	for (var x = x0; x <= x1; ++x) {
		for (var y = y0; y <= y1; ++y) {
			r1 = imgData.data[y*lineOffset + x*4];
			g1 = imgData.data[y*lineOffset + x*4+1];
			b1 = imgData.data[y*lineOffset + x*4+2];
			for (var a = amin; a < amax; a+=1) {
				r2 = imgData.data[(y+a)*lineOffset + x*4];
				g2 = imgData.data[(y+a)*lineOffset + x*4+1];
				b2 = imgData.data[(y+a)*lineOffset + x*4+2];
